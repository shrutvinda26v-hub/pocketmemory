...
function buildShell(color, depth, size, segs, width, depthScale, zMode) {
...
export function buildChameleon(material, colorImage, depthImage) {
  ...
  const front = buildShell(maps.color, maps.depth, size, segs, width, 0.34, "front");
  const back = buildShell(maps.color, maps.depth, size, segs, width, 0.34, "back");
  ...
  const backMesh = new THREE.Mesh(back.geo, backMat);
  const rimMesh = new THREE.Mesh(rim, backMat);
